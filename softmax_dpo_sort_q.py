...
def select_tail_random(
    ...
    """
    Select negatives using a long-tail distribution over sorted candidate rows.
    ...
    Args:
        ...
        tail_direction:
            "low":
                highest probability near lowest metric index.
                Probability decays toward higher metric values.

            "high":
                highest probability near highest metric index.
                Probability decays toward lower metric values.

        temperature:
            controls how sharp or flat the long-tail distribution is.

            smaller temperature -> more concentrated near the head
            larger temperature  -> flatter, more random-like

            Recommended:
                0.10 = very sharp tail
                0.25 = moderate tail
                0.50 = flatter tail
    ...
    for i in range(n):
        ...
        if tail_direction == "low":
            # high weight near q=0, decays as q increases
            weight = pow(2.718281828459045, -q / temperature)

        elif tail_direction == "high":
            # high weight near q=1, decays as q decreases
            weight = pow(2.718281828459045, -(1.0 - q) / temperature)

        else:
            raise ValueError(
                f"Unknown tail_direction={tail_direction}. "
                f"Use 'low' or 'high'."
            )
...
def train(
    ...
    center_q: float = 0.5,
    temperature: float = 0.25,  # only for tail_random select_mode
):
