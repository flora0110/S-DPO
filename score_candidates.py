...
class CandidateScorer:
    ...
    def compute_sequence_logprob(
        ...
        """
        Compute log P(item | prompt).

        sequence_logprob:
            Sum of log probabilities of all item tokens.

        avg_token_logprob:
            Average log probability per item token.

        token_len:
            Number of tokens in item.
        """
        ...
        # First item token is input_ids[query_len].
        # Its probability is predicted by logits at position query_len - 1.
        ...
        seq_logprob = response_token_log_probs.sum()
        avg_token_logprob = response_token_log_probs.mean()

        return {
            "sequence_logprob": safe_float(seq_logprob),
            "avg_token_logprob": safe_float(avg_token_logprob),
            "token_len": item_token_len,
        }
